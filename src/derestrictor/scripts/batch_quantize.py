...
def find_llama_quantize() -> Path | None:
    """Attempt to find the ``llama-quantize`` executable.

    Checks common installation locations and the current ``PATH``.
    """
...
def batch_quantize(
    ...
    llama_quantize_path: Path | None = None,
    num_threads: int | None = None,
    ...
    if llama_quantize_path is None:
        ...
        if llama_quantize_path is None:
            raise FileNotFoundError(
                "Could not find llama-quantize. Please provide path via --llama-quantize or ensure it's in your PATH."
            )
    else:
    ...
    quant_types_sorted = sorted(quant_types, key=lambda x: quant_priority.get(x, 99))
...
def print_summary(results: dict[str, bool], input_path: Path, output_dir: Path):  # noqa: ARG001 - input_path kept for call-site readability
...
def main():
    """Command-line entry point for batch quantization of a GGUF model."""
...
Examples:
  ...
  python batch_quantize.py --list-quants
    ...
    parser.add_argument("input_gguf", type=Path, nargs="?", help="Path to input F16/F32 GGUF file")

    parser.add_argument("-o", "--output-dir", type=Path, help="Output directory (default: same as input file)")

    parser.add_argument(
        "-q", "--quant-types", nargs="+", help=f"Quantization types to generate (default: {' '.join(DEFAULT_QUANTS)})"
    )

    parser.add_argument("--llama-quantize", type=Path, help="Path to llama-quantize executable")

    parser.add_argument("-t", "--threads", type=int, help="Number of threads for quantization")

    parser.add_argument("--name-prefix", type=str, help="Custom prefix for output filenames")

    parser.add_argument("--list-quants", action="store_true", help="List available quantization types and exit")

    parser.add_argument("--all", action="store_true", help="Generate all available quantization types")
    ...
    try:
        ...
        print_summary(results, args.input_gguf, args.output_dir or args.input_gguf.parent)
